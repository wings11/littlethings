...
function Orders() {
  ...
  const handleCreateOrder = async () => {
    ...
    try {
      ...
      const itemsArray = Array.isArray(itemsRes.data.items)
        ? itemsRes.data.items
        : [];
      setItems(itemsArray);
    } catch (err) {
  ...
  return (
    <div className="min-vh-100 bg-white p-4">
      <h2 className="text-2xl font-bold text-pink-custom-300 mb-4">Orders</h2>
              ...
              items.map((item) => (
                ...
                >
                  <span className="text-lg text-gray-800">
                    ...
                    ${item.retail_price}, Wholesale: ${item.wholesale_price},
